...
const testSiteBody = document.body;

// Insert test slug html + add margin to body to account for it
testSiteBody.insertAdjacentHTML("beforeend", testSlugHTML);
testSiteBody.style.marginBottom = "28px";

requestAnimationFrame(() => {
    // Fade in test slug html
    const slug = document.querySelector(".test-slug");
    if (slug) slug.style.opacity = "1";

    const offcanvas = document.querySelector("#main-nav .offcanvas");
    if (!offcanvas) return;

    // Set the media query breakpoint for mobile menu
    const mq = window.matchMedia("(max-width: 991.98px)");

    // Add bottom margin to offcanvas menu to account for test slug
    function updateOffcanvasMargin(e) {
        offcanvas.style.marginBottom = e.matches ? "28px" : "";
    }

    // Run on initial load
    updateOffcanvasMargin(mq);
    // Run every time breakpoint is crossed
    mq.addEventListener("change", updateOffcanvasMargin);
});
